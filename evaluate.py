...
def plot_average_metrics(noisy_scores, enhanced_scores, metric_names, out_dir=RESULTS_DIR):
    ...
    ax.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "avg_metrics_comparison.png"), dpi=150)
    plt.show()
    print("  Saved: avg_metrics_comparison.png")


def plot_improvement_per_file(noisy_scores, enhanced_scores, metric_name, out_dir=RESULTS_DIR):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, f"improvement_{metric_name.lower()}.png"), dpi=150)
    plt.show()
    print(f"  Saved: improvement_{metric_name.lower()}.png")


def plot_improvement_histogram(noisy_scores, enhanced_scores, metric_name, out_dir=RESULTS_DIR):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, f"histogram_{metric_name.lower()}.png"), dpi=150)
    plt.show()
...
def evaluate():
    ...
    for i, (clean_fname, noisy_fname) in enumerate(matched):
        ...
        sf.write(os.path.join(enhanced_dir, clean_fname), enhanced, SAMPLE_RATE)
    ...
    plot_average_metrics(noisy_scores, enhanced_scores, ["STOI", "PESQ", "SNR"], charts_dir)
    for metric in ["STOI", "PESQ", "SNR"]:
    ...
    with open(report_path, "w") as f:
        for metric in ["STOI", "PESQ", "SNR"]:
            n_mean = np.mean(noisy_scores[metric])
            e_mean = np.mean(enhanced_scores[metric])
            f.write(f"{metric}  Noisy: {n_mean:.4f}  Enhanced: {e_mean:.4f}  Δ {e_mean - n_mean:+.4f}\n")
