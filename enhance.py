...
def plot_results(noisy, enhanced, out_dir, clean=None, sr=SAMPLE_RATE):
    ...
    ax.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "waveform_comparison.png"), dpi=150)
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "spectrogram_comparison.png"), dpi=150)
    plt.show()
    ...
    ax.set_ylabel("Frequency bins")
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "difference_spectrogram.png"), dpi=150)
    plt.show()
...
def main():
    ...
    plot_results(noisy[:min_len], enhanced[:min_len], results_dir, clean[:min_len] if clean is not None else None, sr)
